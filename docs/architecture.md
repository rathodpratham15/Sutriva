...
This describes the system as implemented (Phase 0). For the full target architecture see `TraceLens_Master_Plan.md`.

## The temporal model

Everything TraceLens observes is normalized into two shapes, defined once in `packages/core/src/types.ts`:

- **`TemporalEvent`** -- a timestamped, typed observation (`visual`, `audio`, `interaction`, `network`, `console`, `dom`, `terminal`, `git`, `system`) with a description, an optional confidence, and a `source` pointing back to what produced it.
- **`Evidence`** -- a grounded observation backed by a stored `Artifact` (currently a frame image). Evidence exists so a hypothesis can point at something concrete ("this frame, this confidence") rather than a free-text claim.
...
2. `get_timeline` -- the compact, timestamped event list, boundable by time range and `limit`.
3. `get_frame` -- one targeted frame near a timestamp Claude cares about, returned as an actual MCP image content block (this is the one place raw bytes are appropriate -- it's a single small image, not the source video).
4. `analyze_segment` (library-level; not yet exposed over MCP) -- dense on-demand sampling in a narrow window plus a provider call, for when the coarse timeline isn't enough.
...
`packages/providers/src/types.ts` defines `VisionProvider` (`analyzeFrames`, `analyzeSegment`) and `TranscriptionProvider` (reserved for the audio pipeline). Two implementations exist:
...
`packages/timeline` and `packages/storage` never import `@anthropic-ai/sdk` -- only `packages/providers` does. Swapping in another multimodal provider means adding one file there and one branch in `createVisionProvider` (`packages/providers/src/factory.ts`).
...
Browser/terminal/Git instrumentation, the live event bus, the evidence correlation graph, `/debug-video`, and the evaluation harness are designed in the master plan but are later phases -- see the README's Limitations section.
