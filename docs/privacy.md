...
- With the default (`TRACELENS_TRANSCRIPTION_PROVIDER=mock`, used whenever `OPENAI_API_KEY` is unset), audio transcription never leaves your machine -- the extracted audio track (`audio.wav`) stays under `TRACELENS_DATA_DIR` and is only read locally to derive a placeholder segment count.
- With `TRACELENS_TRANSCRIPTION_PROVIDER=openai`, the extracted audio track is sent to the OpenAI Audio API over HTTPS using your own `OPENAI_API_KEY`, only when `inspect_video` (or `tracelens inspect`) actually runs against a video with an audio track -- not automatically, not in the background.
...
- `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are read from the environment (`process.env`) only. Neither is ever written to the SQLite database, logged, or included in any TraceLens error message.
